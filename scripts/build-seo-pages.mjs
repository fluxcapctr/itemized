...
async function main() {
  ...
  // Flat search corpus the homepage SearchBox queries. Procedures,
  // hospitals, glossary terms — everything searchable in one ~50KB bundle
  // loaded synchronously on the homepage.
  const searchIndex = {
    procedures: data.procedures.map((p) => ({
      code: p.code, label: p.label, short: p.short, slug: p._slug,
    })),
    hospitals: [...hospitalById.values()].map((h) => ({
      id: h.id, name: h.name, metro: h.metro, system: h.system || null,
    })),
    glossary: GLOSSARY.map((g) => ({ slug: g.slug, term: g.term, short: g.short })),
  };
  const searchJs = `// Generated by build-seo-pages.mjs. Loaded by Itemized.html for the\n// homepage search box. Combined corpus of ${searchIndex.procedures.length} procedures, ${searchIndex.hospitals.length} hospitals, and ${searchIndex.glossary.length} glossary terms.\nwindow.ITEMIZED_SEARCH = ${JSON.stringify(searchIndex)};\n`;
  // Write to both ui/ (so dev mode loads it) and ui/dist/ (so prod ships it).
  fs.writeFileSync(path.join(UI_DIR, "search-index.real.js"), searchJs);
  fs.writeFileSync(path.join(DIST_DIR, "search-index.real.js"), searchJs);
